...
#ifdef DEBUG
  printf("Transpose : N=%ld, M=%ld, howmany=%d, block0=%ld, block1=%ld\n", in->n0, in->n1, howmany, in->block, out->block);
#endif
...
int main(int argc, char **argv)
{
  ...
  t = t + t_;
  fprintf(stdout, "rank=%d - DFT1 - Time elapsed: %10.3E sec.\n", rank, t_);
  ...
  t_ = (t1.tv_sec - t0.tv_sec) + (t1.tv_usec - t0.tv_usec) / (double)1000000;
  t = t + t_;
  fprintf(stdout, "rank=%d - TRANSPOSE1 - Time elapsed: %10.3E sec.\n", rank, t_);
  ...
  t_ = (t1.tv_sec - t0.tv_sec) + (t1.tv_usec - t0.tv_usec) / (double)1000000;
  t = t + t_;
  fprintf(stdout, "rank=%d - DFT2 - Time elapsed: %10.3E sec.\n", rank, t_);
  ...
  t_ = (t1.tv_sec - t0.tv_sec) + (t1.tv_usec - t0.tv_usec) / (double)1000000;
  t = t + t_;
  fprintf(stdout, "rank=%d - TRANSPOSE2 - Time elapsed: %10.3E sec.\n", rank, t_);
  ...
  fprintf(stdout, "rank=%d - Total Time elapsed: %10.3E sec.\n", rank, t);
